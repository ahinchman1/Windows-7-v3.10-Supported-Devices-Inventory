	...
	public String deviceToString() {
	  ...
	  public void tempDevice() {
		  try {
			 ...
			 buffered.close();
			 // now search for given device and change status
			 for (int row = 0; row < devices.size(); row++) {
				 if (devices.get(row) == this) {
					 devices.get(row).setStatus("Checked Out");
					 System.out.println("Device:" + devices.get(row).toString());
					 System.out.println(devices.get(row));
				 }// if
			 }// for
			 
			 // Save changes to Inventory File
			 ArrayList<String> devicetoString = new ArrayList<String>();
			 for (int onerow = 0; onerow < devices.size(); onerow++) {
				 devicetoString.add(devices.get(onerow).toString());
			 }
			 System.out.println("Updating Inventory....");
			 //System.out.println(devicetoString);
			 /*
			 ...
			 FileWriter fw = new FileWriter(invenfile, true);
			 BufferedWriter bww = new BufferedWriter(fw);
			 
			 bww.write(deviceToString);
			 bww.flush();
			 bww.close();
			 System.out.println("Device status changed in Inventory file.");
			 */
		  } catch (Exception e) {
			  e.printStackTrace();
		  }
...
public class Devices {
	...
	public String deviceToString() {
	  ...
	  public boolean deviceMatch(Devices two) {
		  if (this.getBrand().equals(two.getBrand()) &&
		      this.getDesktopLaptop().equals(two.getDesktopLaptop()) &&
			  this.getMake().equals(two.getMake()) &&
			  this.getType().equals(two.getType()) &&
			  this.getSerial().equals(two.getSerial()) &&
			  this.getStatus().equals(two.getStatus())) {
			  return true;
		  }// if
		  else return false;
	  }// deviceMatch(Devices, Devices)
